...
export default function Home() {
  useEffect(() => {
    ...
    function mouseOverEffect() { this.classList.add("panel-highlight"); }
    function mouseOutEffect() { this.classList.remove("panel-highlight"); }

    panels.forEach(p => p.addEventListener("click", toggleOpen));
    panels.forEach(p => p.addEventListener("transitionend", toggleActive));
    panels.forEach(p => p.addEventListener("mouseover", mouseOverEffect));
    panels.forEach(p => p.addEventListener("mouseout", mouseOutEffect));

    return () => {
      panels.forEach(p => p.removeEventListener("click", toggleOpen));
      panels.forEach(p => p.removeEventListener("transitionend", toggleActive));
      panels.forEach(p => p.removeEventListener("mouseover", mouseOverEffect));
      panels.forEach(p => p.removeEventListener("mouseout", mouseOutEffect));
    };
  }, []);
    ...
    <section className="panels">
      {panelsData.map((panel, i) => (
        <div key={i} className={`panel ${panel.className}`}>
          {panel.lines.map((line, j) => <p key={j}>{line}</p>)}
        </div>
      ))}
    </section>
  );
